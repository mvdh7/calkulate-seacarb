---
title: "Testing revised at function"
author: "May-Lin Paulsen, Jean-Pierre Gattuso..."
...
This script is for testing the version of at.R revised by May-Lin Paulsen. It uses the test titration file alkalinity.csv which must be in the same folder as this Rmd script.
...
# "at" <-
  at <- function(S=35, T=25, CHCl=0.1, HClunit="mol/L",CNaCl=0, HCltemp=20, pHtot="yes", pHTris=NULL, ETris=NULL, weightSample, E, volume){
    ...
    if(HClunit == 'mol/L' | HClunit == 'mol L-1' | HClunit == 'mol/l' | HClunit == 'mol l-1'){
      CHCl <- CHCl/(1000+CHCl*MMHCl)*1000 }# converting from mol/L to mol/kg-sol
    ...
    if(CNaCl == 0){ #based on Handbook of CO2 meas, Dickson et al 2007, chapter 5, paragraph 4.4 
      d = 1.0}
        ...
        MMNaCl<- 22.99 + 35.45
        mHCl  <- CHCl/(1000 - CHCl*MMHCl - CNaCl*MMNaCl)*1000
        mNaCl <- CNaCl/(1000 - CHCl*MMNaCl - CNaCl*MMNaCl)*1000
    ...
    f<-lm(m~F1)
    TA<-f$coefficients[1]*CHCl/m0[1]
    E0est <- z$E/1000-(R*z$Tk/F)*log((-m0*TA+m*CHCl)/(m0+m))
    ...
    y <- (m/m0)
    regr <- nls(y ~ ((At + (St/(1 + Ks*Z/(f*Hprime)))+(Ft/ (1+Kf/(f*Hprime)))+(f*Hprime/Z))/(CHCl-f*Hprime/Z)),start=list(At=TA, f=1))
...
```{r test at, include=FALSE}
d <- read_csv(file = "./alkalinity.csv")
at <- at(S=38, T=d$T, CHCl=0.1, HClunit="mol/L", CNaCl=0, HCltemp=20, pHtot="yes", pHTris=8.13, ETris=-71.544, weightSample = 25.753, E = d$E, volume = d$volume)
```

Calculated total alkalinity = `r format(at$AT*1e6, digits=6)` umol/kg; E0 = `r format(at$E0, digits=6)` mV.